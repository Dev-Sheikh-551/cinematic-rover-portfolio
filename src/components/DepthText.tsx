...
export const DepthText = ({
  ...
  useEffect(() => {
    ...
    const applyTransform = () => {
      if (stage) stage.style.transform = getTransform(current.x, current.y);
    };
    ...
    const handlePointerMove = (event: Event) => {
      ...
      activePointer = true;
      const x = clamp(
        (pe.clientX - (rect.left + rect.width / 2)) / (rect.width * 0.8),
        -1,
        1
      );
      const y = clamp(
        (pe.clientY - (rect.top + rect.height / 2)) / (rect.height * 0.8),
        -1,
        1
      );
