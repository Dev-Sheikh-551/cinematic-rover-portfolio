...
export const OptionWheel: React.FC<OptionWheelProps> = ({
  ...
  const posRef = useRef<number>(selected ?? defaultSelected);
  const targetRef = useRef<number>(selected ?? defaultSelected);
  const rafRef = useRef<number | null>(null);
  const lastRef = useRef<number>(0);
  const cfgRef = useRef<any>({});
  const onChangeRef = useRef(onChange);
  const selectedRef = useRef<number>(selected ?? defaultSelected);
  ...
  const [selectedIndex, setSelectedIndex] = useState<number>(selected ?? defaultSelected);
  ...
  // Sync external controlled selected prop
  useEffect(() => {
